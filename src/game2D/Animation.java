...
public class Animation {
    ...
    private int maxAllowedLoops = Integer.MAX_VALUE; // default: infinite
            ...
            if (animTime >= totalDuration) {
                if (loop) {
                    ...
                    if (loopCount >= maxAllowedLoops) {
                        play = false; // Stop animating
                        loop = false;
                        currFrameIndex = frames.size() - 1; // Stay on last frame
                    } else {
    ...
    public Animation clone() {
        ...
        for (AnimFrame frame : this.frames) {
            copy.addFrame(frame.image, frame.endTime - (copy.totalDuration));
        }
        copy.setLoop(this.loop);
        copy.setLoopCount(this.maxAllowedLoops);
        copy.setAnimationSpeed(this.animSpeed);
        return copy;
    }
